...
def run_single_training(arg_dict, custom_env = None):
    ...
    if custom_env is None:
        ...
        p = subprocess.run("python3 RunSplitTrain.py" + arg_str, shell=True, 
                           check=True, timeout = 2400, env=custom_env)
    return
...
if __name__ == '__main__':
    ...
    for loop.
    This is for Nested-Leave-One-Subject-Out CV.
    
    Example of first call:
    
    $ Python RunNLOSO -d /path/to/data

    Example of another call if run fails for some reasons:

    $ Python RunNLOSO -d /path/to/data -s 130
    ...
    for key in PIPE_args:
        if key == 'pipelineToEval':
            ...
            print( f"{key:15} ==> [i for i in (number of subjects in dataset - 1)]")
        else:
