...
if __name__ == '__main__':
    ...
    help_d = """
    RunSplitTrainSeed run a single training with a specific split 
    ...
    This script adds the possibility to change the random seed.
    ...
    if kfoldstrat in ['lnso', 'nlnso', 'nkfld']:
        ...
        if taskToEval.casefold() == 'sleep':
            partition_list = split.create_nested_kfold_subject_split(Nsubj, Nouter, Ninner, seed=seed)
            ...
            part_a = split.create_nested_kfold_subject_split(a_id, Nouter, Ninner, seed=seed)
            part_c = split.create_nested_kfold_subject_split(c_id, Nouter, Ninner, seed=seed)
            part_f = split.create_nested_kfold_subject_split(f_id, Nouter, Ninner, seed=seed)
            ...
            part_c = split.create_nested_kfold_subject_split(ctl_id, Nouter, Ninner, seed=seed)
            part_p = split.create_nested_kfold_subject_split(pds_id, Nouter, Ninner, seed=seed)
            partition_list_1 = split.merge_partition_lists(part_c, part_p, Nouter, Ninner)
        
            #ds002778 - ID 8 - UCSD
            part_c = split.create_nested_kfold_subject_split([i for i in range(1,17)], Nouter, Ninner, seed=seed)
            part_p = split.create_nested_kfold_subject_split([i for i in range(17,32)], Nouter, Ninner, seed=seed)
            ...
            part_c = split.create_nested_kfold_subject_split(ctl_id, Nouter, Ninner, seed=seed)
            part_p = split.create_nested_kfold_subject_split(psy_id, Nouter, Ninner, seed=seed)
            ...
            partition_list = split.create_nested_kfold_subject_split(subject_list, Nouter, Ninner, seed=seed)
    ...
    # If the partition strategy is of the Leave-ONE-Subject-out type
        ...
        if taskToEval.casefold() == 'sleep':
            ...
            partition_list = split.create_nested_kfold_subject_split(t, Nouter, Ninner, seed=seed)
        
        elif taskToEval == 'parkinson':
            # ds003490 - ID 5 - 3Stim 50
            # ds002778 - ID 8 - UCSD 31
            t = [i for i in range(1,82)]
            partition_list_1 = split.create_nested_kfold_subject_split(t, Nouter, Ninner, seed=seed)   
            partition_list_2 = []
    ...
    # Set the output path
    start_piece_res = 'SeedResults/seed_' + str(seed) + os.sep
    ...
    seed_piece = str(seed).zfill(4)
    
    file_name = '_'.join(
        [cv_piece, task_piece, pipe_piece, freq_piece, mdl_piece, 
         out_piece, in_piece, lr_piece, chan_piece, win_piece, seed_piece
        ]
    )
    results_path = start_piece_res + file_name + '.pickle'
    
    if verbose:
        print('saving results in the following paths')
        print(results_path)
